...
CREATE TABLE users(user_id SERIAL PRIMARY KEY, user_role INT REFERENCES user_role(id_role), name TEXT);
...
CREATE TABLE item (id_item SERIAL PRIMARY KEY,
                ...
                cost INT);
CREATE TABLE comments (id_comments SERIAL PRIMARY KEY, id_item INT REFERENCES item(id_item), name TEXT);
